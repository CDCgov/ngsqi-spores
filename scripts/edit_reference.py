import argparse
from Bio import SeqIO

#chr pos should be entered as a 0 format position

def edit_sequence(ref_file, chromosome, position, variant_seq, output_fa):
    records = []
    for record in SeqIO.parse(ref_file, "fasta"):
        if record.id == chromosome:
            record.seq = record.seq[:position] + variant_seq + record.seq[position+len(variant_seq):]
        records.append(record)
    SeqIO.write(records, output_fa, "fasta")

def main():
    parser = argparse.ArgumentParser(description='Edit a sequence in a FASTA file at a specific position.')
    parser.add_argument('-i', '--ref_file', required=True, help='Input FASTA file')
    parser.add_argument('-c', '--chromosome', required=True, help='chromosomeosome/sequence ID to edit')
    parser.add_argument('-p', '--pos', required=True, type=int, help='position to edit (0-based)')
    parser.add_argument('-s', '--variant_seq', required=True, help='New sequence to insert')
    parser.add_argument('-o', '--output_fa', required=True, help='Output FASTA file')
    
    args = parser.parse_args()
    
    edit_sequence(args.ref_file, args.chromosome, args.pos, args.variant_seq, args.output_fa)
    print(f"Sequence edited successfully. Output written to {args.output_fa}")

if __name__ == '__main__':
    main()